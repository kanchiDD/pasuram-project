...
// 2. Fallback for sections with no anchor section row (2, 4, 5, 11, 26).
//    c() gives the converted section_master name; SECTION_NAMES below is
//    only the last resort if the strings never loaded.
if (!section.label) {
  section.label = c("sec.name." + row.section_id)
               || SECTION_NAMES[row.section_id]
               || "";
}

...
.fnav-label {
  ...
  font-weight:700;
    ...
    /* Lang toggle */
    .farch-lang-wrap { display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap;justify-content:center; }
    .farch-lang-btn { padding:6px 14px;border-radius:16px;border:1.5px solid #c9a84c;background:transparent;color:#c9a84c;font-size:13px;cursor:pointer; }
...
export function archanaiSpinner() {
  return `<div class="farch-spinner" style="background:#1a0a00;min-height:60vh;">
    ...
    <div class="farch-loading-text">${uiText("archLoading")}</div>
  </div>`;
}
...
function floatNav() {
  return `<div class="fstar-float-nav">
    ...
    <div class="fnav-item">
      ...
      <div class="fnav-label">${uiText("home")}</div>
    </div>

    <div class="fnav-item">
      <button onclick="window.location.href='tree.html'">↩</button>
      <div class="fnav-label">${uiText("backPlain")}</div>
    </div>

    <div class="fnav-item">
      <button onclick="window.scrollTo({top:0,behavior:'smooth'})">⬆</button>
      <div class="fnav-label">${uiText("top")}</div>
    </div>

    <div class="fnav-item">
      <button onclick="window.scrollBy({top:-window.innerHeight*.85,behavior:'smooth'})">◀</button>
      <div class="fnav-label">${uiText("up")}</div>
    </div>

    <div class="fnav-item">
      <button onclick="window.scrollBy({top:window.innerHeight*.85,behavior:'smooth'})">▶</button>
      <div class="fnav-label">${uiText("down")}</div>
    </div>
...
export async function renderFullDivyadesamArchanai() {
  ...
  if (!desams.length) {
    return `<div class="farch-page">
      <div class="farch-title">${uiText("archTitle")}</div>
  ...
  function _getName(d) { return d["name_" + _currentLang] || d.name_ta || ""; }
  function _getNv(d)   { return d["nv_"   + _currentLang] || d.nv_ta   || ""; }

  // Font map
  const FONT_MAP = {
    ta: '"Noto Sans Tamil","Latha",serif',
    hi: '"Noto Sans Devanagari","Noto Sans",serif',
    te: '"Noto Sans Telugu","Noto Sans",serif',
    kn: '"Noto Sans Kannada","Noto Sans",serif',
    ml: '"Noto Sans Malayalam","Noto Sans",serif',
  };
  const GFONT = {
  ...
  function _loadFont(lang) {
    if (!GFONT[lang]) return;
    const id = "gfont-" + lang;
    if (document.getElementById(id)) return;
    const lk = document.createElement("link");
    lk.id = id; lk.rel = "stylesheet";
    lk.href = "https://fonts.googleapis.com/css2?family=" + GFONT[lang] + "&display=swap";
  ...
  window.farchSetLang = function(lang) {
    ...
    if (root) root.style.fontFamily = FONT_MAP[lang] || FONT_MAP.ta;
    _farchShow(_idx);
  };
  ...
  function _farchShow(i) {
    ...
    setTimeout(() => {
      ...
      name.textContent = _getName(d);
      nv.textContent   = _getNv(d);
  ...
  function _farchNext() {
    if (_idx >= DESAMS.length - 1) {
      ...
      if (btn) { btn.textContent = uiText("play"); btn.classList.remove("active"); }
  ...
  window.farchToggle = function() {
    ...
    const btn = document.getElementById("farch-play-btn");
    if (_playing) {
      if (btn) { btn.textContent = uiText("pause"); btn.classList.add("active"); }
      _farchStartTimer();
    } else {
      if (btn) { btn.textContent = uiText("play"); btn.classList.remove("active"); }
  ...
  window.farchGoTo = function() {
    const n = parseInt(prompt(uiText("archGoToPrompt")), 10);
...
<div class="farch-page" id="farch-root">
  <div class="farch-title">${uiText("archTitle")}</div>
  <div class="farch-subtitle">${uiText("archSubtitle")}</div>
  ...
  <div class="farch-controls" style="flex-direction:column;gap:10px;">
    ...
    <div style="display:flex;gap:14px;justify-content:center;">
      <button class="farch-btn" onclick="farchPrev()">${uiText("previous")}</button>
      <button class="farch-btn" onclick="farchNext()">${uiText("next")}</button>
    </div>
  </div>
  ...
  <div class="farch-controls" style="margin-top:6px;">
    ...
    <button class="farch-btn" onclick="farchGoTo()">${uiText("goTo")}</button>

...
// ─────────────────────────────────────────────
export async function renderGhoshtiSetup(meta, appDiv, onSave) {
  ghoshtiMeta    = meta   || {};
...
function buildSetupHTML() {
  ...
  <div class="recital-wrap">
    ...
    <div class="recital-nav-row">
      <span class="recital-back-link" onclick="window._ghoshtiBack()">← Back</span>
      <div class="recital-title" style="margin:0">Select Pasurams</div>
      <span></span>
    </div>
    ...
    <div class="r-pasuram-box">
      ...
      <div class="r-pasuram-row">
        ...
        <button class="r-pasuram-btn" onclick="window._ghoshtiLookupPasuram()">Add</button>
      </div>
    ...
    <button class="recital-btn-primary" style="width:100%;margin-top:8px"
            onclick="window._ghoshtiSave()">
      Save Ghoshti &amp; Get Link 🙏
    </button>
  ...
  <div class="r-modal-overlay" id="r-modal-overlay"
       onclick="window._ghoshtiCloseModalOutside(event)">
    <div class="r-modal-box">
      ...
      <div class="r-modal-back" id="r-modal-back"
           onclick="window._ghoshtiModalBack()">← Back</div>
...
function findSuperiorConflict(entity_type, entity_id, section_id, pathu_id, is_child) {
  for (const item of selectedItems) {
    ...
    // Full pathu: pathu_id=null, is_child=false, entity_id=first_child_id
    if (item.entity_type === "pathu" && item.pathu_id === null && !item.is_child) {
      const firstChildId = item.entity_id;
      if (is_child && entity_type === "pathu"
          && pathu_id === firstChildId
          && Number(item.section_id) === Number(section_id)) {
        return { type: "block", existing: item };
      }
      // No thirumozhi block — thirumozhi sections are separate; null===null would false-match
    }
...
function findInferiorItems(entity_type, entity_id, section_id, pathu_id, is_child, global_no_start, global_no_end, pathu_no) {
  ...
  for (const item of selectedItems) {
    if (newItemIsSection) {
      ...
      // Child: pathu_id non-null (full pathu has pathu_id=null)
...
function renderMetaBar() {
  if (!ghoshtiMeta) return;
  const hEl = document.getElementById("g-meta-heading");
  const wEl = document.getElementById("g-meta-when");
  if (hEl) hEl.textContent = ghoshtiMeta.heading || "";
  if (wEl && ghoshtiMeta.date && ghoshtiMeta.time) {
    const d = new Date(`${ghoshtiMeta.date}T${ghoshtiMeta.time}`);
    wEl.textContent = d.toLocaleDateString("en-IN", {
...
function renderCatalogIntoDOM() {
  ...
  const KOIL_HTML = `<div class="r-thousand-group">
    ...
    <div class="r-section-card" onclick="window._ghoshtiAddKoil(1)">
      <div class="r-section-name">கோயில் திருமொழி</div>
      <div style="color:#b38b2e">+</div>
    </div>
    <div class="r-section-card" onclick="window._ghoshtiAddKoil(2)">
...
function renderSelected() {
  ...
  displayItems.forEach((item, i) => {
    ...
    html += `<div class="r-selected-item" draggable="true"
      ondragstart="window._ghoshtiDragStart(${origIdx})"
      ondragover="window._ghoshtiDragOver(event)"
      ondrop="window._ghoshtiDragDrop(${origIdx})"
      ondragend="window._ghoshtiDragEnd()">
      ...
      <span style="color:#c0392b;cursor:pointer;font-weight:700;flex-shrink:0"
            onclick="window._ghoshtiRemoveItem('${item.entity_type}',${item.entity_id})">✕</span>
    </div>`;
  });
  el.innerHTML = html;
}
  ...
  if (frame.level === "section") {
    html += `<div class="r-modal-option">
      <input type="checkbox" ${isSelected("section", frame.section_id) ? "checked" : ""}
             onchange="window._ghoshtiToggleSection(
      ...
      for (const p of frame.data.items) {
        html += `<div class="r-modal-option">
          <input type="checkbox" ${selectedItems.some(i=>i.entity_type==="pathu"&&i.entity_id===p.pathu_id&&i.pathu_id===null&&!i.is_child)?"checked":""}
                 onchange="window._ghoshtiTogglePathuCheck(
                   ${frame.section_id},'${escHtml(frame.section_name)}',
          ...
          <span class="r-modal-drill"
                onclick="window._ghoshtiOpenPathu(
                  ${frame.section_id},'${escHtml(frame.section_name)}',
      ...
      for (const t of frame.data.items) {
        ...
        html += `<div class="r-modal-option">
          <input type="checkbox" ${isSelected("thirumozhi", t.thirumozhi_id) ? "checked" : ""}
                 onchange="window._ghoshtiToggleThirumozhiCheck(
    ...
    for (const t of frame.data.items) {
      ...
      html += `<div class="r-modal-option">
        <input type="checkbox" ${isSelected("pathu", t.pathu_id) ? "checked" : ""}
               onchange="window._ghoshtiToggleThirumozhiPathuCheck(
...
// ─────────────────────────────────────────────
// LOAD EXISTING GHOSHTI PLAN
// ─────────────────────────────────────────────
async function loadExistingGhoshtiPlan(plan_id) {
  const mobile = localStorage.getItem("mobile");
  if (!mobile || !plan_id) return;
  try {
    ...
    for (const dbItem of data.items) dbItemMap.set(`${dbItem.entity_type}_${dbItem.entity_id}`, dbItem);
    selectedItems = labelData.labels.map(item => {
      ...
      if (dbItem.is_rettai) {
        ...
        const rettaiLabel = item.label.includes("இரட்டை") ? item.label : `${item.label} — இரட்டை`;
        return { entity_type:"rettai_group", entity_id:item.entity_id,
          rettai_key:`${item.entity_type}_${item.entity_id}`,
          rettai_source:{ entity_type:item.entity_type, entity_id:item.entity_id },
          label:rettaiLabel, global_no_start:dbItem.global_no_start||item.global_no_start||0,
          section_id:dbItem.section_id||item.section_id||null,
          pathu_id:isChild ? restoredPathuId : null, is_child:isChild, pasurams:[] };
      }
      // Full pathu: pathu_id=null in DB. Child: pathu_id=non-null.
      // Do NOT fallback to entity_id — that destroys the null signal for full pathu.
      const restoredPathuId = (item.pathu_id !== undefined && item.pathu_id !== null)
        ? item.pathu_id
        : (dbItem.pathu_id !== undefined ? dbItem.pathu_id : null);
      return { entity_type:item.entity_type, entity_id:item.entity_id, label:item.label,
    ...
    if (rettaiItems.length) {
      Promise.all(rettaiItems.map(async item => {
        try {
          ...
          if (item.is_child) { params.set("is_child","1"); if (item.global_no_start) params.set("gns",item.global_no_start); }
          const d=(await (await fetch(`${WORKER}/recital/rettai?${params}`)).json());
          const count=d.pasurams?.length||0;
          if (count>0) item.label=item.label.replace(/ — இரட்டை.*$/,"") + ` — இரட்டை (${count} pasurams)`;
        } catch(e) {}
      })).then(()=>renderSelected());
    }
...
// WINDOW BINDINGS
// ─────────────────────────────────────────────
function registerGhoshtiBindings() {
  ...
  window._ghoshtiOpenSection = openSectionModal;

  // Section checkbox in modal
  window._ghoshtiToggleSection = (section_id, section_name, global_no_start, checked) => {
    if (checked) {
  ...
  window._ghoshtiTogglePathuCheck = (section_id, section_name, pathu_id, pathu_name, pathu_no, global_no_start, checked) => {
    if (checked) {
  ...
  window._ghoshtiToggleThirumozhiCheck = (thirumozhi_id, label, global_no_start, global_no_end, section_id, pathu_id, checked) => {
    if (checked) {
  ...
  window._ghoshtiToggleThirumozhiPathuCheck = (pathu_id, label, global_no_start, global_no_end, section_id, parent_pathu_id, checked) => {
    if (checked) {
  ...
  window._ghoshtiPickRettai = async () => {
    if (!pendingItem) return;
  ...
  window._ghoshtiCancelPopup = () => {
    document.getElementById("r-popup-overlay").classList.remove("open");
    pendingItem = null;
  };

  // Pathu drill down
  window._ghoshtiOpenPathu = async (section_id, section_name, pathu_no, pathu_name, pathu_id, global_no_start) => {
  ...
  window._ghoshtiCloseModalOutside = (e) => {
    if (e.target.id === "r-modal-overlay") window._ghoshtiCloseModal();
  };

  window._ghoshtiRemoveItem = (entity_type, entity_id) => {
    removeItem(entity_type, Number(entity_id));
  };

  window._ghoshtiDragStart = (i)      => { dragSrcIndex = i; };
  window._ghoshtiDragEnd   = ()       => { dragSrcIndex = null; };
  window._ghoshtiDragOver  = (e)      => { e.preventDefault(); };
  window._ghoshtiDragDrop  = (target) => {
  ...
  window._ghoshtiLookupPasuram = async () => {
    ...
    try {
      ...
      preview.innerHTML = `
        <div style="background:#fef3d0;border:1px solid #e8c060;
          ...
          ${previewLines}
          <button onclick="window._ghoshtiConfirmPasuram(
                    ${data.global_no},
                    '${escHtml(label)}',
                    ${data.section_id || 0},
                    ${data.pathu_id   || 0},
  ...
  window._ghoshtiConfirmPasuram = (global_no, label, section_id, pathu_id, pathu_no) => {
  ...
  window._ghoshtiSave = async () => {
    ...
    if (!mobile) {
      if (saveMsg) { saveMsg.style.color="#c0392b"; saveMsg.textContent="Please sign in first."; saveMsg.style.display="block"; }
      return;
    }
    ...
    const saveBtn = document.querySelector('button[onclick="window._ghoshtiSave()"]');
    ...
    try {
      ...
      if (!planData.success) throw new Error(planData.error || "Plan save failed");

      // Step 2: Create OR update ghoshti session
      let ghoshtiData;
      if (ghoshtiMeta.ghoshti_id) {
        // Editing existing ghoshti — just update plan_id link
        const ghoshtiRes = await fetch(`${WORKER}/recital/ghoshti`, {
          method:  "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ghoshti_id:   ghoshtiMeta.ghoshti_id,
            mobile,
            plan_id:      planData.plan_id,
            ghoshti_name: ghoshtiMeta.heading,
            start_time
          })
        });
        ghoshtiData = await ghoshtiRes.json();
        if (!ghoshtiData.success) throw new Error(ghoshtiData.error || "Ghoshti update failed");
        ghoshtiData.ghoshti_id = ghoshtiMeta.ghoshti_id;
        ghoshtiData.link = `https://arulicheyal.org/ghoshti.html?id=${ghoshtiMeta.ghoshti_id}`;
      } else {
        // New ghoshti
        const ghoshtiRes = await fetch(`${WORKER}/recital/ghoshti`, {
          method:  "POST",
        ...
        ghoshtiData = await ghoshtiRes.json();
        if (!ghoshtiData.success) throw new Error(ghoshtiData.error || "Ghoshti creation failed");
      }

      isDirty = false;
      if (saveMsg) saveMsg.style.display = "none";

      // Step 3: Callback to ghoshti.html
      if (onSaveCallback) {
        onSaveCallback({
          ghoshti_id: ghoshtiData.ghoshti_id,
          link:       ghoshtiData.link,
          expires_at: ghoshtiData.expires_at,
          heading:    ghoshtiMeta.heading,
          start_time
        });
      }
    } catch(e) {
      ...
      if (saveMsg) { saveMsg.style.color="#c0392b"; saveMsg.textContent="Error: "+e.message; saveMsg.style.display="block"; }
    }
  };
}

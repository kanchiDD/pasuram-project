...
console.log("LEVEL BEFORE RENDER:", state.level);
  ...
  switch (state.level) {
    ...
    case "STAR_PASURAM":
      topbar.style.display = "flex";
      app.innerHTML = starSpinner();
      renderFullStarPasuram(state.starName || null).then(html => {
        app.innerHTML = html;
      });
      break;

    case "DIVYADESAM_ARCHANAI":
      topbar.style.display = "none";
      app.innerHTML = archanaiSpinner();
      renderFullDivyadesamArchanai().then(html => {
        app.innerHTML = html;
        // Functions registered on window by renderFullDivyadesamArchanai
        // Init must run AFTER innerHTML is set so DOM elements exist
        requestAnimationFrame(() => {
          if (typeof window._farchInit === "function") window._farchInit();
        });
      });
      break;

    case "AZHWAR_THIRUNATCHATHRA":
      topbar.style.display = "flex";
      app.innerHTML = thirunatchathraSpinner();
      renderFullAzhwarThirunatchathra().then(html => {
        app.innerHTML = html;
      });
      break;
